...
namespace sc
{
      ...
      : ipu_channel_id_(channel_id), camera_identity_(camera_identity)
  {
    ...
    if (isInitalized)
    {
      std::cout << "InferenceChannel is already initialized";
    }
    else
    {
      // Find TPU device.
      ...
      // std::cout<< "Found TPU at " << device.path << std::endl;

      // // Load model.
      // auto model = tflite::FlatBufferModel::BuildFromFile(kCoralModelImgPath.c_str());
      // if (!model) {
      //   std::cerr << "Cannot read model from " << kCoralModelImgPath << std::endl;
      //   return;
      // }

      // std::cout<< "Model: " << kCoralModelImgPath.c_str() << " buffers" << std::endl;
      ...
      // std::cout<< "Interpreter: " << interpreter->tensors_size() << " tensors" << std::endl;

      // auto *delegate = edgetpu_create_delegate(device.type, device.path, nullptr, 0);

      // std::cout<< "Delegate: " << device.type << " TPU at " << device.path << std::endl;
      // interpreter->ModifyGraphWithDelegate(delegate);

      // std::cout<< "ModifyGraphWithDelegate" << std::endl;
      ...
      // std::cout<< "Tensors allocated: " << interpreter->tensors_size() << " allocated" << std::endl;

      // // Set interpreter input.
      // const auto *input_tensor = interpreter->input_tensor(0);
      // std::cout<< "input_tensor->type: " << input_tensor->type << " kTfLiteUInt8: " << kTfLiteUInt8 << std::endl;
      // std::cout<< "image_height: " << kImageHeight << std::endl;
      // std::cout<< "image_width: " << kImageWidth << std::endl;
      // std::cout<< "image_bpp: " << kImageChannels << std::endl;
      // std::cout<< "input_tensor->dims->data[0] (1): " << input_tensor->dims->data[0] << std::endl;
      // std::cout<< "input_tensor->dims->data[1] height: " << input_tensor->dims->data[1] << std::endl;
      // std::cout<< "input_tensor->dims->data[2] widht: " << input_tensor->dims->data[2] << std::endl;
      // std::cout<< "input_tensor->dims->data[3] bpp: " << input_tensor->dims->data[3] << std::endl;

      // std::cout<< "Input tensor: " << input_tensor->dims->data[1] << "x" << input_tensor->dims->data[2] << "x"
      //           << input_tensor->dims->data[3] << std::endl;
      // std::cout<< "InferenceChannel initialization complete! Setting initalized boolean to true";

      // std::cout<< "Address of interpreter: " << &interpreter << std::endl;
      // std::cout<< "Address of the interpreter object: " << interpreter.get() << std::endl;

      InferenceChannel::isInitalized = true;
      std::cout << "InferenceChannel initialization complete!";
    }
  }
  ...
  sc::Status InferenceChannel::RunImageInference(Image *image)
  {
    ...
    // static const std::string image_file = "/opt/320x320_column_121923_AP6303280348.bmp";
    ...
    static const std::string image_file = "/opt/320x192_column_121923_AP6303280348.bmp";

    std::vector<uint8_t> image_buffer;

    std::cout << "Image: " << image->width << "x" << image->height << "x" << image->number_of_bytes << std::endl;
    cv::Mat img(image->height, image->width, CV_8UC3, image->virtual_address);

    if (!isInit)
    {
      std::cout << "Initializing inference pipeline..." << std::endl;
    ...
    std::cout << "Output tensor: " << output_tensor.dims->data[0] << "x" << output_tensor.dims->data[1] << "x"
              << output_tensor.dims->data[2] << std::endl;
    // Process the detections
    process_detection_output(output_tensor, image_width, image_height);
    ...
    auto postprocess_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(end_process - end_time).count();
    ...
    std::cout << "Postprocess completed successfully in " << postprocess_time_ms << " ms." << std::endl;
