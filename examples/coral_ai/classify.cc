...
std::string GetLabel(const std::vector<std::string>& labels, int label) {
  if (label >= 0 && label < static_cast<int>(labels.size())) return labels[label];
...
std::vector<float> Dequantize(const TfLiteTensor& tensor) {
  ...
  for (int i = 0; i < static_cast<int>(tensor.bytes); ++i)
    result[i] = tensor.params.scale * (data[i] - tensor.params.zero_point);
  return result;
}

std::vector<std::pair<int, float>> Sort(const std::vector<float>& scores, float threshold) {
  std::vector<const float*> ptrs(scores.size());
  std::iota(ptrs.begin(), ptrs.end(), scores.data());
  auto end = std::partition(ptrs.begin(), ptrs.end(), [=](const float* v) { return *v >= threshold; });
  std::sort(ptrs.begin(), end, [](const float* a, const float* b) { return *a > *b; });

  std::vector<std::pair<int, float>> result;
  result.reserve(end - ptrs.begin());
  for (auto it = ptrs.begin(); it != end; ++it) result.emplace_back(*it - scores.data(), **it);
  return result;
}
}  // namespace
...
int main(int argc, char* argv[]) {
  if (argc != 5) {
    std::cerr << argv[0] << " <model_file> <label_file> <image_file> <threshold>" << std::endl;
    return 1;
  }
  ...
  auto image = ReadBmpImage(image_file.c_str(), &image_width, &image_height, &image_bpp);
  if (image.empty()) {
  ...
  std::cout << "Model: " << model_file.c_str() << " buffers" << std::endl;
  ...
  std::cout << "Input tensor copied, invoking tensor..." << std::endl;
  ...
  std::cout << "Inference invoked" << std::endl;

  // Get interpreter output.
  auto results = Sort(Dequantize(*interpreter->output_tensor(0)), threshold);

  std::cout << "Results: " << results.size() << " above threshold" << std::endl;

  for (auto& result : results)
    std::cout << std::setw(7) << std::fixed << std::setprecision(5) << result.second << GetLabel(labels, result.first)
              << std::endl;
